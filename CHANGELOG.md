...
- `references/verification.md` — a green baseline is recorded in the ledger before the first dispatch; when it cannot be green, the already-failing checks are listed so later runs compare against that list rather than zero. (Adapted from `obra/superpowers` `using-git-worktrees`.)
