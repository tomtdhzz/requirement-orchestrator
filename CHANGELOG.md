...
- **New concept document, `docs/freeze-before-fanout.md`** (zh/en): the three freezes — contract, scope, fact — with an observable criterion each, and a real run where those criteria stopped a wrong three-way split and caught a startup panic the compiler could not see.
...
- **A contract that is a produced/external artifact's schema is grounded before it is frozen**: when the shape cannot be known until a real instance exists (a parser's output, a real API response, a scraped structure), sample one first — a read-only spike, or one piloted slice when the artifact is what you are building — and take its schema as a verified fact. Freezing a guessed schema makes every task inherit the wrong contract; sampling to learn a shape is grounding, not `execute`.
