...
- **READMEs state the positioning and show it**: why the project exists (one person; the bottleneck is acceptance, not throughput), a 60-second walkthrough of one spoken request turning into four checkable artifacts (a thin spec, a recorded parallel verdict, `run`/`expect` verification, completion judged on the diff), the three pillars, and an at-a-glance comparison against a skills collection and a host's orchestrator mode. `agents/openai.yaml` short description synced.
