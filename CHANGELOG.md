...
- **READMEs rewritten for a cold reader** (236 → 130 lines each): two concrete failure stories and the criterion that stops each, a 30-second start, an at-a-glance comparison, `when not to use it`, the four artifacts a request turns into, dogfooding numbers from this repo's own history, three pillars, and a roadmap with an explicit `never` list. Duplicated restatements of `SKILL.md` (control loop, progress surface, boundaries, layout) were dropped — `SKILL.md` is the single source.
